...
<script>
	...
	export default {
		...
		props: {
			...
			// 图片压缩比例
			// 可以从 0 到 100 的区间内选择图片的质量。如果超出取值范围，将会使用默认值 92
			compressionRatio: {
				type: Number,
				default: 92
			}
		...
		methods: {
			...
			async uploadFile(index = 0) {
				...
				this.lists[index].error = false;
				this.uploading = true;
				let filePath = this.lists[index].url;
				// #ifdef H5
				const resp = await rotateImg(this.lists[index].file, this.compressionRatio)
				if (resp) {
					filePath = resp
					this.lists[index].url = filePath
				}
				// #endif
